...
function CheckInOut() {
  ...
  const handleCheckOut = async (equipmentID) => {
    ...
    const equipmentCheckedOut = itemsToCheckOut.find(
    ...
    await setDoc(
      ...
      {
        checkedOutItems: [
          ...reservation.checkedOutItems,
          { id: equipmentID, name: equipmentCheckedOut.name },
        ],
      },
      { merge: true },
    );
    ...
    const itemsToCheckOutUpdated = itemsToCheckOut.filter(
      (equipment) => equipment.id !== equipmentID,
    );
  ...
  const handleCheckIn = async (equipmentID) => {
    ...
    const equipmentCheckedIn = reservation.checkedOutItems.find(
      (equipment) => equipment.id === equipmentID,
    );
    const checkedOutItemsUpdated = reservation.checkedOutItems.filter(
      (equipment) => equipment.id !== equipmentID,
    );
    const reservationRef = doc(db, "reservations", reservationID);
    await setDoc(
      ...
      {
        checkedInItems: [
          ...reservation.checkedInItems,
          { id: equipmentID, name: equipmentCheckedIn.name },
        ],
    ...
    if (overdue) {
      await setDoc(
        ...
        {
          overdueItems: [
            ...reservation.overdueItems,
            {
              id: equipmentID,
              name: equipmentCheckedIn.name,
    ...
    //update the availability in the inventory collection for that item
    const equipmentRef = doc(db, "inventory", equipmentID);
    await setDoc(equipmentRef, { availability: "available" }, { merge: true });
  };
  ...
  const handleAddItem = async () => {
    ...
    const itemsToCheckOutList = reservationData.equipmentIDs.filter(
      (equipment) =>
        !reservationData.checkedOutItems.some(
          (item) => item.id === equipment.id,
        ) &&
        !reservationData.checkedInItems.some(
          (item) => item.id === equipment.id,
        ),
    );
  ...
  return (
    <div className="bg-white m-8 p-8 rounded-lg relative">
      <div className="pl-2 pr-2">
          ...
          {activeReservation && reservation && (
            <div className="flex flex-col space-y-2 md:flex-row md:space-x-2 shrink-0">
              <ExtendReservationPopup
                handleEdit={() => handleExtendReservation()}
                reservation={reservation}
                reservationID={reservationID}
              />
              <ConfirmationPopup
                handle={() => handleCancelReservation()}
        ...
        <h3 className="text-lg sm:text-xl pb-2 ">
          {reservation && formatDate(reservation.startDate)} -{" "}
          {reservation && formatDate(reservation.endDate)}
        </h3>
        <h3 className="text-lg sm:text-xl pb-2">
          <span className="font-semibold">Team:</span> {reservation?.team}
        </h3>
        <h3 className="text-lg sm:text-xl">
          <span className="font-semibold">Items Held:</span>{" "}
          {reservation?.equipmentIDs?.length}
        </h3>
            ...
            <div className="flex-1 p-4 rounded">
              ...
              {itemsToCheckOut.map((item, index) => (
                <div key={index} className="flex items-center space-x-8 mb-4">
                  ...
                  <div className="bg-[#ECECEC] w-full p-2 rounded-md border-2 border-black flex items-center justify-between m-2">
                    <div className="bg-white rounded-md p-2 w-2/3 text-center lg:text-xl sm:text-lg text-sm break-words">
                      <h3>{item.name}:</h3>
                      <h3 className="break-all">{item.id}</h3>
                    </div>
                    <CheckOutInPopUp
                      handleCheckOutIn={() => handleCheckOut(item.id)}
                      checkOut={true}
                      correctID={item}
                    />
                  </div>
              ...
              {reservation?.checkedOutItems?.map((item, index) => (
                <div key={index} className="flex items-center space-x-4 mb-4">
                  <div className="bg-[#ECECEC] w-full p-2 rounded-md border-2 border-black flex items-center justify-between">
                    ...
                    <CheckOutInPopUp
                      handleCheckOutIn={() => handleCheckIn(item.id)}
                      checkOut={false}
                      correctID={item}
                    />
                  </div>
        ...
        <div className="flex justify-center items-center flex-col">
          ...
          {reservation?.checkedInItems?.map((item, index) => (
            <div
              key={index}
              className="bg-[#ECECEC] w-3/4 sm:w-1/2 p-2 rounded-md border-2 border-black content-center mb-4"
            >
